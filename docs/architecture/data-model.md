...
- ExecutionGrant / ExecutionCaptureDecision / ExecutionLease：Day5
...
Day4 的 protected-original 后端路径只接受通过严格 schema 校验且完整绑定
...
上述能力是后端存储边界，不是 Day5 在线 capture authority 的扩展。Day5
...
Lease provenance 接线属于 Day18，因此当前依赖这些证据的 XSS 仍为
Inconclusive。
...
- Cookie、Token 和响应数据按最小必要原则处理，普通文本证据只保存脱敏内容。受保护原件生成独立 metadata-only derivative，并由 `0010` 封套执行配额、`retentionUntil`、crypto-erase 和审计；这不授权 Day5 在线路径采集真实 DOM/截图。
