...
- contracts 只校验 ID 格式；是否存在和版本是否可用由冻结 DefinitionRegistry 决定。一般模块能否主动执行还必须经过 Day7 qualification record 与 ActivationCatalog；Day2 仅为 `sqli/xss/ssrf/idor` 固定 V1 实现保留一个封闭、可审计的 `legacy-v1` 临时兼容例外。
- DefinitionRegistry 是可信 Composition Root，不支持从知识库、模型输出或运行时下载的任意代码动态注册。除固定四类 `legacy-v1` 兼容映射外，`registered-only` 不能被当前扫描选择执行；该例外不改变其 `activationStatus=registered`，也不得对外声明 qualified/supported。
...
5. 注册时校验 ID/版本唯一、交叉引用完整、未知 Capability 拒绝、声明 active 的 technique 具有负对照和最小 Evidence role；L2 先校验 `requiresTestObject/requiresCleanup` 声明，正式 TestObject/CleanupProtocol 和 FixtureSuite/qualification 深层一致性分别由 Day8/Day7 完成。缺 qualification 不是定义注册失败，而是保持 registered-only；仅固定四类 V1 可由下述临时兼容例外继续执行。
...
7. 默认扫描 families 从 contracts 移到 Application 配置；只默认启用已明确兼容的四个 legacy module。Application 执行入口必须同时检查 DefinitionRegistry、runtime support 与 ActivationCatalog。执行条件为“精确环境已 qualified”，或命中固定 `sqli/xss/ssrf/idor` 的 `legacy-v1` definition/runtime 映射与 Application allowlist；不能因为定义存在就主动运行。
...
9. 开放字符串 ID 前修复 legacy runtime 的安全兜底：CreateScan、start/resume 和 Candidate 执行前均做 DefinitionRegistry + runtime support + Activation/固定兼容 allowlist 三门禁；Coordinator 的 IDOR 必须是显式分支，最终未知 family 直接失败，不能把任何未知 ID 当 IDOR 主动执行。pause/cancel 仍允许安全停止未知历史扫描。
...
新增一个漏洞目录项不再要求修改 `z.enum`；DefinitionRegistry 能拒绝不完整或越权的 Bundle。除固定四类 `legacy-v1` 临时兼容映射外，registered-only 不能执行；旧四类数据和前端类型仍保持兼容，且该例外必须在 Day7 产生有效 qualification record 后被替换并移除。
