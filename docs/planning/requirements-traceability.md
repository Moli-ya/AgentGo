...
| AG-SEC-002 | L1 仅 reviewed 只读最小动作；POST/PUT/PATCH 或未知副作用默认 L2；family 名称不能降级风险。 | 主动探测 §2；Backend V2 §6/16 | `已有` | Day6 | [security-policy tests](../../packages/security-policy/src/security-policy.test.ts) 覆盖基础风险判定；Day3 [Inventory classifier/service](../../packages/application/src/inventory-service.ts) 将人工 `reviewStatus` 与确定性 `executionClass` 分离，[Coordinator regression](../../packages/application/src/scan-coordinator.test.ts) 证明 target seed 在任何 HTTP/模型/AgentRun 前等待 review，legacy 执行只消费同一 reviewed、active、standard HTTP、无 body、纯 query 变体的 URL、selector 与参数语义；legacy runner 不会隐式提供的自定义 header 失败关闭。 | 当前产品 L2 仍只有调用方布尔语义；可信批准/TestObject/cleanup 完成前真实目标 L2 不可用。Day4～Day6 仍需统一 compiler/lease/wire 绑定。 |
...
| AG-WP-004 | 纯 RequestCompiler、Template/Resolved/Wire 三阶段哈希与 EvidenceCapturePolicy。 | [Day4](Day4.md) | `后续波次` | Day4 | 首个纯函数切片见 [RequestCompiler](../../packages/application/src/request-compiler.ts)、[EvidenceCapturePolicy](../../packages/application/src/evidence-capture-policy.ts) 与 [进度档案](../audits/day4-progress-2026-07-19.md)；V1 HTTP 构造/Evidence 保存仍位于 [execution service](../../packages/application/src/execution-service.ts) | `partial / in_progress`：已建立合同、三阶段证明与最小化策略，但 compatibility adapter、redirect 重编译及 capture-before-store 接入未完成；旧执行/证据路径仍不受新策略约束。 |
