...
>
> 最近事实更新：2026-07-30；Day4 的 `protected-original` 后端实现与终验
> 已收口，事实见
> [Day4 终验档案](../audits/day4-final-review-2026-07-30.md)。Day5 已在
> 最终代码上完成
> [post-Day4 全回归](../audits/day5-post-day4-review-2026-07-30.md)；
> 2026-07-29 的结果继续作为变更前历史证据保留。
...
| AG-ARCH-006 | 扫描冻结 scope、module、technique、rule、evidence profile、capability 与 Registry snapshot；恢复不得偷用新版。 | Backend V2 §3.3；数据模型 §7 | `Day3` | Day3 | Scope 由 [workflow contracts](../../packages/contracts/src/workflow.ts) 冻结；module/technique/rule/evidence/capability/Registry/环境由 [snapshot contracts](../../packages/contracts/src/inventory.ts)、[snapshot verifier](../../packages/application/src/scan-module-snapshot.ts)、[DB schema/migration](../../packages/db/src/schema.ts) 与 [integration tests](../../packages/application/src/scan-module-snapshot.integration.test.ts) 封存并核对。 | 2026-07-18 已验收；缺失、损坏、未封存或 `legacy-unknown` 的历史语义进入 awaiting-user/Inconclusive。该快照不替代 Day7 qualification，也不开放新执行能力。 |
...
| AG-SEC-001 | 每次实际 I/O 前重新核对 origin/path/port/IP/redirect/identity/有效期/预算，未知或歧义输入 fail closed。 | 主动探测 §3；Backend V2 §16 | `已有` | Day6 | Day5 [ExecutionAuthority/Guard](../../packages/application/src/execution-policy.ts)、[HTTP runner](../../packages/http-runner/src/index.ts)、[历史完成档案](../audits/day5-completion-2026-07-28.md) 与 [post-Day4 复验](../audits/day5-post-day4-review-2026-07-30.md) 已覆盖 exact-wire、单次 Lease、每跳 fresh authority 与 DNS guard。 | Day5 已正式闭环；Day6 仍负责 scan 级 request/RPM/concurrency/bytes 原子 reserve/settle、精细地址分类与响应资源门禁，不得把 Day5 单步限制外推为 Day6 已完成。 |
| AG-SEC-002 | L1 仅 reviewed 只读最小动作；POST/PUT/PATCH 或未知副作用默认 L2；family 名称不能降级风险。 | 主动探测 §2；Backend V2 §6/16 | `已有` | Day6 | [security-policy tests](../../packages/security-policy/src/security-policy.test.ts) 覆盖基础风险判定；Day3 [Inventory classifier/service](../../packages/application/src/inventory-service.ts) 将人工 `reviewStatus` 与确定性 `executionClass` 分离，[Coordinator regression](../../packages/application/src/scan-coordinator.test.ts) 证明 target seed 在任何 HTTP/模型/AgentRun 前等待 review，legacy 执行只消费同一 reviewed、active、standard HTTP、无 body、纯 query 变体的 URL、selector 与参数语义；legacy runner 不会隐式提供的自定义 header 失败关闭。 | 当前产品 L2 仍只有调用方布尔语义；可信批准/TestObject/cleanup 完成前真实目标 L2 不可用。Day4/Day5 已通过顺序终验；Day6 仍负责原子预算与资源门禁。 |
...
| AG-SEC-007 | Evidence 默认不可变、内容哈希可校验；脱敏生成派生物并保留审计关系，捕获前遵循最小化策略。 | 数据模型 §3/6；威胁模型 §9 | `已有` | Day4 | [EvidenceStore](../../packages/db/src/evidence-store.ts)、[EvidenceCapturePolicy](../../packages/application/src/evidence-capture-policy.ts) 与 [Day4 终验档案](../audits/day4-final-review-2026-07-30.md) 对应的 2026-07-30 实现已覆盖默认 hash-only/脱敏写入，以及受固定计划约束的加密原件、后端访问、hash、配额、保留、派生与到期清理。 | 最终门禁结果只由终验档案确认；该后端能力没有把 DOM/截图接入 legacy XSS 实际执行链，需要 DOM/截图原件的 XSS 仍为 Inconclusive，实际集成属于 Day18。 |
| AG-SEC-008 | 每次主动执行记录 scope/identity、Proposal/Policy、请求/响应摘要、Evidence、side effect、cleanup、版本与结束状态。 | 主动探测 §7/9；数据模型 §7 | `已有` | Day5 | [ExecutionService](../../packages/application/src/execution-service-v2.ts)、[execution repository](../../packages/db/src/execution-repository.ts)、[Day5 历史完成档案](../audits/day5-completion-2026-07-28.md) 与 [post-Day4 复验](../audits/day5-post-day4-review-2026-07-30.md) 绑定 Template/Resolved/Wire proof、Grant、Lease、ToolCall、Interaction、capture decision、Evidence role 和终态。 | Day5 已在最终代码上完成顺序终验；L2 side effect/Cleanup Receipt 仍由 Day8/10 负责。 |
...
| AG-V1-007 | Policy 检查 scope、method、identity、DNS/redirect 与声明预算；Day5 实际执行还必须消费 Grant/单次 Lease，但 L2 仍无可信批准。 | V1 审计；主动探测 §7 | `已有` | Day10 | [security-policy](../../packages/security-policy/src/index.ts)、[execution policy](../../packages/application/src/execution-policy.ts)、[ADR-0006](../adr/0006-deterministic-execution-authority.md) | 无可信 ActorContext/ApprovalPort、TestObject 与 cleanup；`approvalBundleRef` 只是合同占位，真实目标 L2 保持禁用。 |
| AG-V1-008 | HTTP Runner 是受 Guard 约束的单跳 transport；Browser Runner 是受 Lease 约束的断网离线渲染。 | DAY0 §3；ADR-0006 | `已有` | Day13 | [HTTP Runner](../../packages/http-runner/src/index.ts)、[Browser Runner](../../packages/browser-runner/src/index.ts)、统一 [ExecutionPort](../../packages/application/src/execution-port.ts) 及测试 | Coordinator 未利用复杂 body；无 BrowserNetworkBroker、Session/CSRF 或复杂协议 adapter。HTTP redirect 由 Application 每跳 fresh authority，不由 Runner 自动跟随。 |
| AG-V1-009 | Evidence 内容寻址、hash 校验、capture-decision-bound 最小化摘要、受保护原件后端与三态报告已存在。 | V1 审计；数据模型 | `已有` | Day4 | [EvidenceStore](../../packages/db/src/evidence-store.ts)、[EvidenceCapturePolicy](../../packages/application/src/evidence-capture-policy.ts)、[reporting](../../packages/reporting/src/index.ts) 与 [Day4 终验档案](../audits/day4-final-review-2026-07-30.md) 对应的实现。 | 最终测试结果不得从本表推断；Day18 尚未把 DOM/截图接入 XSS 实际执行、审阅和确认链，当前 marker 仍不能据此 Confirmed。 |
...
| AG-WP-003 | 唯一 scan-scoped Inventory、RequestVariant/Source/Selector/Codec/Transport、opaque refs 与 module snapshot。 | [Day3](Day3.md) | `Day3` | Day3 | [Inventory contracts](../../packages/contracts/src/inventory.ts)、[canonicalization/redaction](../../packages/domain/src/inventory.ts)、[Application service](../../packages/application/src/inventory-service.ts)、[schema/migration/repository](../../packages/db/src/schema.ts)、[snapshot verifier](../../packages/application/src/scan-module-snapshot.ts) 与 [完成审计](../audits/day3-completion-2026-07-18.md)。 | 2026-07-18 已验收；Day3 只交付统一真源与安全结构。通用请求编译属 Day4，Vault/TestObject 生命周期属 Day9，OpenAPI/HAR/静态/浏览器 producer 属 Day11～Day13。 |
| AG-WP-004 | 纯 RequestCompiler、Template/Resolved/Wire 三阶段哈希与 EvidenceCapturePolicy。 | [Day4](Day4.md) | `后续波次` | Day4 | [RequestCompiler](../../packages/application/src/request-compiler.ts)、[legacy adapter](../../packages/application/src/legacy-v1-request-compiler-adapter.ts)、[EvidenceCapturePolicy](../../packages/application/src/evidence-capture-policy.ts) 已接入统一执行/审计/恢复链；2026-07-30 的实现补齐 protected-original 后端。 | 实现已收口；正式状态、最终命令和数字仅见 [Day4 终验档案](../audits/day4-final-review-2026-07-30.md)。不包含 Day18 DOM/截图实际执行链。 |
| AG-WP-005 | ExecutionGrant、单次 Lease、实际 wire 恒等复核与唯一 ExecutionPort。 | [Day5](Day5.md) | `后续波次` | Day5 | [execution contracts](../../packages/contracts/src/execution.ts)、[authority](../../packages/application/src/execution-authority.ts)、[guard](../../packages/application/src/execution-policy.ts)、[ExecutionPort](../../packages/application/src/execution-port.ts)、DB migrations 0007～0009、[历史完成档案](../audits/day5-completion-2026-07-28.md) 与 [post-Day4 复验](../audits/day5-post-day4-review-2026-07-30.md)。 | `completed / uncommitted`；已在 Day4 最终代码上完成全量回归，旧结果只作为历史基线。 |
